...
class Test_extract_data(unittest.TestCase):
	def test_get_paper_id(self):
		self.assertEqual(dic.get_paper_id(), 'ABC00001', "Should be 'ABC00001'")
	def test_get_title(self):
		self.assertEqual(dic.get_title(), 'Covid', "Should be Covid")
	def test_gat_text(self):
		self.assertEqual(dic.get_text(), 'Covid is coron virus', "Should be Covid is coron virus")
...
class Test_preprocess_data(unittest.TestCase):
	...
	def test_remove_special_character(self):
		self.assertEqual(preprocess.remove_special_character("hello number @1 and #2"), 'hello number 1 2', "Should be 'hello number 1 2'")
